"""
ducksearch/collector_search.py  —  Job search via DuckDuckGo (no API key)

Outputs to ducksearch/ folder:
  ducksearch/latest_jobs_ddg.csv
  ducksearch/latest_jobs_ddg.md
  ducksearch/jobs_seen_ddg.json
  ducksearch/current_jobs_ddg.json
"""
...
SEEN_FILE     = os.path.join(BASE_DIR, "jobs_seen_ddg.json")
CURRENT_FILE  = os.path.join(BASE_DIR, "current_jobs_ddg.json")
OUTPUT_MD     = os.path.join(BASE_DIR, "latest_jobs_ddg.md")
OUTPUT_CSV    = os.path.join(BASE_DIR, "latest_jobs_ddg.csv")
WINDOW_HOURS  = 24
...
def main() -> None:
    ...
    print(json.dumps({"new_from_ddg": len(new_jobs), "total_in_window": len(current_jobs)}, indent=2))
